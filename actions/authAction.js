...
export async function loginAction(payload) {
  try {
    ...
    const sessionExpireMinutes = parseInt(
      process.env.SESSION_EXPIRE_MINUTES || "60",
      10
    );
    ...
    logger.info(
      `User Logged (in) Username:${user.username}, UserID:${serializeUserId}, Role: ${user.role}`
    );
...
export async function logoutAction() {
  ...
  try {
    ...
    logger.info(
      `User Logged (out) Username:${verification.username}, UserID:${verification.userId}, Role: ${verification.role}`
    );
